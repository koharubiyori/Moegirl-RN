...
export const load = id => dispatch((dispatch, getState) =>
...
export const incrementLoad = () => dispatch((dispatch, getState) =>{
  var state = getState().comment
  return getComments(state.pageId).then(data => dispatch({ type: INCREMENT_DATA, posts: data.posts }))
})

export const del = id => dispatch({ type: DEL, id })

export const setLikeStatus = (id, zan) => dispatch({ type: SET_LIKE_STATUS, id, zan })
...
export default function(Element){
  return connect(
    state => ({ state }),
    dispatch => ({ 
      comment: { load, incrementLoad, del, setLikeStatus }
    })
  )(Element)
}

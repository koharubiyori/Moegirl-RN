...
function SearchPage(props: PropsWithChildren<Props>) {
  ...
  async function removeSearchHistory(title: SearchHistory) {
    await dialog.confirm.show({ content: i.recentSearch.removeSearchHistory.check })
    const newSearchHistory = searchHistory.filter(item => item.keyword !== title.keyword)
    setSearchHistory(newSearchHistory)
    storage.set('searchHistory', newSearchHistory)
  }
    ...
    <ViewContainer>
      ...
      : 
        <RecentSearch 
          ...
          onLongPressTitle={removeSearchHistory}
        />
      }
    </ViewContainer>
  )
